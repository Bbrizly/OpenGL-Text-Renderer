        ...
        if (line.rfind("info", 0) == 0) {
            ...
            while (iss >> key) {
                if (key.find("lineHeight=") == 0) {
                    ...
                    m_pages = std::stoi(key.substr(6));
                    std::cout << "Total pages: " << m_pages << std::endl;
                }
            }
            ...
            while (iss >> key) {
                if (key.find("id=") == 0) {
                    ...
                    ch.page = std::stoi(key.substr(5)) + m_pageOffset; //FOR ARRAY TEXTURE YAYAYAYA
                }
            }
...
const unordered_map<std::pair<char, char>, int, PairHash>& MiniFont::GetKerning() const {
    return m_kerning;
}
