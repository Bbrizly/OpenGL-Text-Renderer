...
void Font::LoadFont(const string& fontPath) {
    ...
    while (std::getline(file, line)) {
        if (line.rfind("info", 0) == 0) {
            // Parse info tag (optional, for debugging or metadata)
            ...
            // Parse page tag to load texture
            size_t filePos = line.find("file=");
            // cout<<"AA: "<<line.rfind("file", 0)<<endl;
            if (filePos != std::string::npos) {
                std::string textureFile = line.substr(filePos + 6); // Skip "file=\""
                textureFile.pop_back(); // Remove trailing quote
    ...
    // Ensure the fallback character exists
